<x-form.modal.card persistent title="Editar Acceso" wire:model.live="showModal" align="center">
    ...
    <x-slot name="footer">
        <div class="flex justify-between">
            <div>
                <x-form.button negative label="Eliminar" wire:click.prevent="delete" spinner="delete"
                    onclick="return confirm('¿Estás seguro de que deseas eliminar este acceso?')" />
            </div>
            <div class="flex gap-2">
                <x-form.button flat label="Cancelar" wire:click="closeModal" />
                <x-form.button primary label="Actualizar" wire:click.prevent="save" spinner="save" />
            </div>
        </div>
    </x-slot>
</x-form.modal.card>
